...
event0_fn(DOM_EventListener *listener, DOM_Event *evt)
{
	...
	MSG("event0_fn Fired! -- type=%s,target=%s,currentTarget=%s,eventPhase=%d,bubbles=%d,timeStamp=%llu", evt->type, evt->target->nodeName, evt->currentTarget->nodeName, evt->eventPhase, evt->bubbles, evt->timeStamp);
...
{
	MSG("event1_fn Fired! -- type=%s,target=%s,currentTarget=%s,eventPhase=%d,bubbles=%d,timeStamp=%llu", evt->type, evt->target->nodeName, evt->currentTarget->nodeName, evt->eventPhase, evt->bubbles, evt->timeStamp);
	listener = NULL;
}
