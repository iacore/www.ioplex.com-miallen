	...
	<xsl:variable name="subs">
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:value-of select="concat(substring-after(../pre,concat(' ',$prev,',')),substring-after(../pre,concat(' ',$prev,')')),substring-after(../pre,concat('*',$prev,')')),substring-after(../pre,concat('*',$prev,',')))"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<xsl:variable name="intr" select="concat(substring-before($subs,concat(' ',@name,',')),substring-before($subs,concat(' ',@name,')')),substring-before($subs,concat('*',@name,')')),substring-before($subs,concat('*',@name,',')))"/>
